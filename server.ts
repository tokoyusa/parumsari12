...
// and handles cleanup of Vercel serverless function routing prefixes like /api/index.ts or /index.ts
app.use((req, res, next) => {
  if (process.env.VERCEL) {
    let originalUrl = req.url || "";
    // Remove serverless entrypoint remnants if present
    originalUrl = originalUrl.replace(/^\/api\/index\.ts/i, "").replace(/^\/index\.ts/i, "");
    
    // Ensure it starts with /api if it's an API route and not root
    if (!originalUrl.startsWith("/api") && originalUrl !== "/" && originalUrl !== "") {
...
// 3. Request Body String/Buffer Auto-Decoder Middleware
app.use((req, res, next) => {
  if (req.body) {
...
// API health check endpoint
  ...
  // --- AI MARKETING KIT GENERATOR & FALLBACKS ---
  ...
  function getGeminiClient(): GoogleGenAI | null {
    if (!aiClient) {
      ...
      if (!key) {
        console.warn("GEMINI_API_KEY environment variable is not defined. AI Marketing Kit will run in local simulation fallback mode.");
        return null;
      }
  ...
  function getLocalFallbackKit(productName: string, category: string, sellingPoints: string, price: string) {
    const cleanName = productName || "Produk UMKM Unggulan";
    const points = sellingPoints || "Diproduksi secara higienis, rasa otentik, dan mendukung ekonomi desa.";
    const priceStr = price ? `Rp ${Number(price).toLocaleString('id-ID')}` : "Harga bersahabat";

    return {
      description: `✨ *${cleanName.toUpperCase()}* - Keaslian Rasa dari Desa Tegalsari, Kandeman, Batang! ✨\n\nProduk pilihan dari kategori *${category || "Karya Lokal"}* yang dibuat langsung dengan cinta oleh warga lokal Desa Tegalsari. Diramu menggunakan bahan baku terbaik yang segar dan alami.\n\n🔥 *Keunggulan Utama:*\n- ${points.split(',').join('\n- ')}\n- 100% Produk Asli UMKM Batang\n- Tanpa bahan pengawet berbahaya\n\n💰 *Harga:* ${priceStr}\n\nDukung terus pertumbuhan ekonomi warga dengan membeli produk dari tetangga kita sendiri!`,
      copywriting: `📢 PROMO SPESIAL KARYA TETANGGA! 📢\n\nSiapa lagi yang mau merasakan keaslian *${cleanName}*? 😍\n\nDiproduksi segar setiap hari langsung dari dapur UMKM Desa Tegalsari, Kandeman, Batang. Sangat cocok dinikmati bersama keluarga tercinta atau dijadikan buah tangan khas Batang.\n\n🚗 *Kabar Gembira:* Khusus warga Desa Tegalsari dan sekitarnya, kami melayani pengiriman langsung ke depan rumah dengan sistem *COD (Bayar di Tempat)* via Kurir Mandiri Desa! Aman, nyaman, dan langsung sampai!\n\nYuk order sekarang sebelum kehabisan! Klik link di profil ya! 👇👇\n\n#Parumsari #UMKMTegalsari #KandemanBatang #BeliKaryaTetangga #BatangHits`,
      bannerSlogan: `Rasa Otentik Khas Tegalsari!`,
      simulatedKeywords: "wooden table product, fresh ingredients, healthy food, traditional fabric"
    };
  }

  app.post("/api/ai/marketing-kit", async (req, res) => {
    const { productName, category, sellingPoints, price } = req.body;
    ...
    console.log(`[AI Marketing Kit] Processing request for: ${productName}`);

    try {
      const ai = getGeminiClient();

      if (!ai) {
        const fallback = getLocalFallbackKit(productName, category, sellingPoints, price);
        return res.json({
          success: true,
      ...
      const prompt = `Buatkan deskripsi produk, copywriting promosi medsos, slogan banner, dan kata kunci gambar untuk:
Nama Produk: ${productName}
Kategori: ${category || "Umum"}
Keunggulan: ${sellingPoints || "Alami, lezat, berkualitas"}
Harga: ${price || "Hubungi penjual"}

Berikan respon Anda dalam format JSON murni sesuai schema yang ditentukan.`;

      const response = await ai.models.generateContent({
        model: "gemini-3.5-flash",
        contents: prompt,
        config: {
          systemInstruction: `Anda adalah pakar pemasaran digital & copywriter profesional Indonesia yang berfokus pada pemberdayaan UMKM (Usaha Mikro, Kecil, dan Menengah). Tugas Anda adalah membuat materi pemasaran (marketing kit) yang sangat menarik, persuasif, dan bernuansa lokal untuk produk UMKM dari Desa Tegalsari, Kecamatan Kandeman, Kabupaten Batang.
Materi harus mengedukasi pembeli, menonjolkan kualitas lokal, keaslian produk, dan mendorong gerakan "Bangga Beli Karya Tetangga".

Gunakan format JSON respon berikut:
{
  "description": "Deskripsi produk e-commerce yang lengkap, terstruktur, ramah SEO, dengan poin-poin manfaat, cara penyajian/penggunaan, legalitas (bila relevan), dan hashtags.",
  "copywriting": "Keterangan promosi media sosial (Instagram/Facebook/WhatsApp) yang emosional dan persuasif, menggunakan emoji yang menarik, menjelaskan bahwa produk ini diproduksi langsung di Desa Tegalsari, Kandeman, Batang, dengan sistem pengiriman COD yang diantar langsung oleh kurir lokal.",
  "bannerSlogan": "Slogan promosi singkat (3-6 kata) yang sangat catchy untuk dicetak pada foto produk/banner.",
  "simulatedKeywords": "3 kata kunci bahasa Inggris yang relevan dipisahkan koma untuk pencarian gambar latar belakang di Unsplash (misal: 'chips wood', 'coconut drink fresh', 'batik fabric pattern')."
}`,
      ...
      console.error("[AI Marketing Kit Error] Error generating content:", error.message);
      const fallback = getLocalFallbackKit(productName, category, sellingPoints, price);
      return res.json({
        success: true,
  ...
  async function getPakasirConfig(clientProject?: string, clientApiKey?: string) {
    ...
    // Helpers to check if a value is custom and valid (i.e. not empty and not sandbox)
    const isCustomKey = (key?: string) => {
      const cleaned = cleanVal(key);
      return cleaned !== "" && cleaned !== sandboxKey && cleaned !== "xxx123";
    };

    const isCustomProject = (proj?: string) => {
      const cleaned = cleanVal(proj);
      return cleaned !== "" && cleaned !== sandboxProject && cleaned !== "xxx123";
    };

    // Rule 1: Prioritize client-passed keys IF they are custom/genuine
    if (isCustomKey(clientApiKey) && isCustomProject(clientProject)) {
      return {
        apiKey: cleanVal(clientApiKey),
        project: cleanVal(clientProject),
        enabled: true
      };
    }

    // Rule 2: Prioritize Vercel / server-side environment variables IF they are custom/genuine
    if (isCustomKey(envApiKey) && isCustomProject(envProjectName)) {
      return {
        apiKey: envApiKey,
    ...
    // Otherwise, try to resolve from Supabase or use fallbacks
    let finalProject = sandboxProject;
    let finalApiKey = sandboxKey;
    let finalEnabled = true;

    if (isCustomKey(envApiKey)) {
      finalApiKey = envApiKey;
    }
    if (isCustomProject(envProjectName)) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data && data.length > 0) {
            ...
            if (isCustomKey(dbApiKey)) {
              finalApiKey = cleanVal(dbApiKey);
            }
            if (isCustomProject(dbProject)) {
              finalProject = cleanVal(dbProject);
            }
    ...
    // Fallbacks if one is custom but the other is not
    if (isCustomProject(clientProject) && !isCustomProject(finalProject)) {
      finalProject = cleanVal(clientProject);
    }
    if (isCustomKey(clientApiKey) && !isCustomKey(finalApiKey)) {
      finalApiKey = cleanVal(clientApiKey);
    }
  ...
  app.get(["/api/pakasir/debug-config", "/pakasir/debug-config"], async (req, res) => {
    try {
  ...
  app.post(["/api/pakasir/create", "/pakasir/create"], async (req, res) => {
  ...
  app.post(["/api/pakasir/simulate", "/pakasir/simulate"], async (req, res) => {
  ...
  app.post(["/api/pakasir/cancel", "/pakasir/cancel"], async (req, res) => {
    const { order_id, amount, project, api_key } = req.body;
    if (!order_id || !amount) {
      return res.status(400).json({ success: false, message: "Missing required parameters: order_id, amount" });
    }
  ...
  app.post(["/api/pakasir/disbursement/create", "/pakasir/disbursement/create"], async (req, res) => {
  ...
  app.post(["/api/pakasir/disbursement/simulate", "/pakasir/disbursement/simulate"], async (req, res) => {
  ...
  app.get(["/api/pakasir/status/:order_id", "/pakasir/status/:order_id"], async (req, res) => {
...
function parseAboutUsMetadata(aboutUs: string) {
...
app.get("/manifest.json", async (req, res, next) => {
  ...
  if (supabaseUrl && supabaseKey) {
    try {
      ...
      console.error('Error fetching manifest settings:', e);
    }
  }
...
app.get("/icon.svg", async (req, res, next) => {
  const supabaseUrl = process.env.VITE_SUPABASE_URL;
  const supabaseKey = process.env.VITE_SUPABASE_ANON_KEY;
  ...
  if (supabaseUrl && supabaseKey) {
    try {
      ...
      console.error('Error serving dynamic icon:', e);
    }
  }
  next();
});
...
app.get("/splash.svg", async (req, res, next) => {
  const supabaseUrl = process.env.VITE_SUPABASE_URL;
  const supabaseKey = process.env.VITE_SUPABASE_ANON_KEY;
  ...
  if (supabaseUrl && supabaseKey) {
    try {
      ...
      console.error('Error serving dynamic splash:', e);
    }
  }
  next();
});
