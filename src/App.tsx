...
import { TegalsariProfileView } from './components/tegalsari-profile-view';
import MarketingContentView from './components/MarketingContentView';
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'katalog' | 'tentang' | 'vendor' | 'admin' | 'profil' | 'pesanan' | 'transaksi-pesanan' | 'profil-tegalsari' | 'ai-tool' | 'marketing-nib' | 'marketing-pirt' | 'marketing-halal' | 'marketing-pelatihan' | 'marketing-whatsapp'>('katalog');
  ...
  useEffect(() => {
    ...
    // Detect URL active tab e.g. ?tab=pesanan
    const urlTab = params.get('tab');
    if (urlTab) {
      const validTabs = ['katalog', 'tentang', 'vendor', 'admin', 'profil', 'pesanan', 'transaksi-pesanan', 'profil-tegalsari'];
      if (validTabs.includes(urlTab)) {
        setActiveTab(urlTab as any);
        console.log('🔄 Mengarahkan tab aktif dari URL:', urlTab);
      }
    }
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (isPakasir) {
        // Trigger Pakasir Transaction Creation in background so Pakasir registers the order_id, without blocking the client redirect
        try {
          handleCreatePakasirTransaction(selectedPakasirMethod, order.id, grandTotal).catch(e => {
            console.warn('[Background Pakasir Registration]', e);
          });
        } catch (e) {}

        const project = appSettings?.pakasir_merchant_id || appSettings?.pakasir_project_name || 'pasar-tegalsari';
        const qrisOnly = selectedPakasirMethod === 'qris' ? '&qris_only=1' : '';
        const redirectUrl = window.location.origin + '/?tab=pesanan';
        const payUrl = `https://app.pakasir.com/pay/${project}/${Math.round(grandTotal)}?order_id=${order.id}&redirect=${encodeURIComponent(redirectUrl)}${qrisOnly}`;

        // Directly redirect to Pakasir payment checkout page
        window.location.href = payUrl;
        return;
      } else {
  ...
  return (
    <div className="min-h-screen pb-24 lg:pb-16 flex flex-col font-sans selection:bg-emerald-100 selection:text-emerald-900 bg-slate-50 text-slate-900">
      ...
      <header className="bg-emerald-700 text-white border-b border-emerald-800 sticky top-0 z-30 shadow-md">
        <div className="max-w-7xl mx-auto px-2.5 sm:px-4 md:px-6 h-16 flex items-center justify-between gap-1.5 sm:gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-4 text-xs font-semibold">
            ...
            <button
              onClick={() => setActiveTab('profil-tegalsari')}
              className={`px-3 py-1.5 rounded-lg transition-all duration-200 cursor-pointer ${
                activeTab === 'profil-tegalsari' 
                  ? 'bg-emerald-800/90 text-white font-bold shadow-inner border border-emerald-600/60' 
                  : 'text-emerald-100 hover:text-white hover:bg-emerald-650/40'
              }`}
            >
            ...
            {/* Dropdown Menu Marketing Tool */}
            <div 
              className="relative"
              onMouseEnter={() => setShowMarketingDropdown(true)}
              onMouseLeave={() => setShowMarketingDropdown(false)}
            >
              <button
                type="button"
                className={`px-3 py-1.5 rounded-lg transition-all duration-200 font-bold flex items-center gap-1.5 text-amber-300 hover:text-white hover:bg-emerald-650/40 cursor-pointer ${
                  ['marketing-nib', 'marketing-pirt', 'marketing-halal', 'marketing-pelatihan', 'marketing-whatsapp'].includes(activeTab)
                    ? 'bg-emerald-800/90 text-white font-bold shadow-inner border border-emerald-600/60'
                    : ''
                }`}
              >
              ...
              {showMarketingDropdown && (
                <div className="absolute left-0 mt-1 w-56 bg-white border border-slate-100 rounded-xl shadow-lg py-2 z-50 animate-fade-in text-xs font-semibold text-slate-700">
                  ...
                  >
                    ...
                    AI TOOL (Asisten Produk)
                  </a>

                  <div className="h-px bg-slate-100 my-1" />

                  {/* Submenu Perijinan Berusaha with Sub-submenus */}
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setActiveTab('marketing-nib');
                      setShowMarketingDropdown(false);
                    }}
                    className={`w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-emerald-50 hover:text-emerald-700 transition cursor-pointer ${
                      activeTab === 'marketing-nib' ? 'bg-emerald-50 text-emerald-700 font-bold' : ''
                    }`}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setActiveTab('marketing-pirt');
                      setShowMarketingDropdown(false);
                    }}
                    className={`w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-emerald-50 hover:text-emerald-700 transition cursor-pointer ${
                      activeTab === 'marketing-pirt' ? 'bg-emerald-50 text-emerald-700 font-bold' : ''
                    }`}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setActiveTab('marketing-halal');
                      setShowMarketingDropdown(false);
                    }}
                    className={`w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-emerald-50 hover:text-emerald-700 transition cursor-pointer ${
                      activeTab === 'marketing-halal' ? 'bg-emerald-50 text-emerald-700 font-bold' : ''
                    }`}
                  >
                  ...
                  {/* Submenu Info Pelatihan */}
                  <button
                    type="button"
                    onClick={() => {
                      setActiveTab('marketing-pelatihan');
                      setShowMarketingDropdown(false);
                    }}
                    className={`w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-emerald-50 hover:text-emerald-700 transition cursor-pointer ${
                      activeTab === 'marketing-pelatihan' ? 'bg-emerald-50 text-emerald-700 font-bold' : ''
                    }`}
                  >
                  ...
                  {/* Submenu Grub Whatsapp */}
                  <button
                    type="button"
                    onClick={() => {
                      setActiveTab('marketing-whatsapp');
                      setShowMarketingDropdown(false);
                    }}
                    className={`w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-emerald-50 hover:text-emerald-700 transition cursor-pointer ${
                      activeTab === 'marketing-whatsapp' ? 'bg-emerald-50 text-emerald-700 font-bold' : ''
                    }`}
                  >
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-6 flex-1 w-full space-y-6">
        ...
        {activeTab === 'tentang' && <AboutUMKM appSettings={appSettings} initialSubTab="tentang" />}

        {/* VIEW 2C: AI Marketing Lab (AI Tool) Tab */}
        {activeTab === 'ai-tool' && <AboutUMKM appSettings={appSettings} initialSubTab="ai-marketing" />}

        {/* VIEW 2B: Tegalsari Profile Tab */}
        {activeTab === 'profil-tegalsari' && <TegalsariProfileView appSettings={appSettings} />}

        {/* VIEW 2D: Marketing & Perijinan Sub-Pages */}
        {activeTab === 'marketing-nib' && <MarketingContentView appSettings={appSettings} type="nib" />}
        {activeTab === 'marketing-pirt' && <MarketingContentView appSettings={appSettings} type="pirt" />}
        {activeTab === 'marketing-halal' && <MarketingContentView appSettings={appSettings} type="halal" />}
        {activeTab === 'marketing-pelatihan' && <MarketingContentView appSettings={appSettings} type="pelatihan" />}
        {activeTab === 'marketing-whatsapp' && <MarketingContentView appSettings={appSettings} type="whatsapp" />}
        ...
        {activeTab === 'pesanan' && !currentProfile && (
          <div className="max-w-md mx-auto text-center py-16 px-6 bg-white rounded-2xl border border-slate-100 shadow-xl space-y-4 my-8">
            <div className="w-16 h-16 bg-emerald-50 rounded-full flex items-center justify-center mx-auto text-emerald-600">
              <ClipboardList className="w-8 h-8" />
            </div>
            <h2 className="text-lg font-bold text-slate-800">Silakan Masuk Terlebih Dahulu</h2>
            <p className="text-xs text-gray-500 max-w-xs mx-auto">Anda perlu masuk atau mendaftar untuk melihat daftar transaksi belanja atau pesanan Anda.</p>
            <button
              onClick={() => {
                setAuthError('');
                setShowLoginModal(true);
              }}
              className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs transition shadow-md cursor-pointer"
            >
              Masuk / Daftar Akun
            </button>
          </div>
                        ...
                        {!vendorMembActivePayment ? (
                          ...
                          <div className="space-y-3 flex flex-col items-center">
                            ...
                            <div className="w-full">
                              <a
                                href={`https://app.pakasir.com/pay/${appSettings?.pakasir_merchant_id || appSettings?.pakasir_project_name || 'pasar-tegalsari'}/${Math.round(vendorMembActivePayment.original_amount || vendorMembActivePayment.amount)}?order_id=${vendorMembActivePayment.order_id}&redirect=${encodeURIComponent(window.location.origin + '/?tab=vendor')}&qris_only=1`}
                                target="_blank"
                                rel="noopener noreferrer"
            ...
            <div className="p-4 space-y-4 overflow-y-auto flex-1">
              ...
              <div className="text-center space-y-3">
                ...
                <div className="pt-1 text-center">
                  <a
                    href={`https://app.pakasir.com/pay/${appSettings?.pakasir_merchant_id || appSettings?.pakasir_project_name || 'pasar-tegalsari'}/${Math.round(activePakasirPayment.original_amount || activePakasirPayment.amount)}?order_id=${activePakasirPayment.order_id}&redirect=${encodeURIComponent(window.location.origin + '/?tab=pesanan')}${activePakasirPayment.payment_method === 'qris' ? '&qris_only=1' : ''}`}
                    target="_blank"
                    rel="noopener noreferrer"
          ...
          <div className="relative flex flex-col w-72 max-w-xs h-full bg-emerald-950 text-white shadow-2xl transition-transform duration-300 transform translate-x-0 overflow-y-auto">
            ...
            <div className="p-4 flex-1 space-y-5">
              ...
              <div className="space-y-1">
                ...
                {/* Mobile Menu Marketing Tool with Submenus */}
                <div className="space-y-1 pt-1 border-t border-emerald-800/40">
                  <span className="px-3 text-[10px] font-bold text-emerald-300 uppercase tracking-wider block mb-1">
                    Marketing Tool
                  </span>
                  ...
                  <a
                    ...
                    rel="noopener noreferrer"
                    onClick={() => setShowMobileSidebar(false)}
                    className="w-full flex items-center gap-2.5 pl-6 pr-3 py-1.5 rounded-xl text-xs font-bold transition text-amber-300 hover:text-white hover:bg-emerald-900/50 animate-fade-in"
                  >
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 pl-6 pr-3 py-1.5 rounded-xl text-xs font-semibold transition cursor-pointer ${
                      activeTab === 'marketing-nib' 
                        ? 'bg-emerald-800 text-amber-300 border border-emerald-700/65 font-bold shadow-inner' 
                        : 'text-emerald-100 hover:text-white hover:bg-emerald-900/50'
                    }`}
                  >
                    <Award className="w-3.5 h-3.5 shrink-0 text-emerald-300" />
                    Nomor Induk Berusaha (NIB)
                  </button>
