...
import { Check, X, Shield, Users, Settings, Megaphone, Info, RefreshCw, AlertCircle, Sparkles, UploadCloud, Trash2, Plus, CreditCard, Map, Truck, UserCheck, Award, Zap, FileText, MapPin, Building, Image as ImageIcon } from 'lucide-react';
...
export default function AdminPanel({ currentProfile, appSettings, onRefreshSettings }: AdminPanelProps) {
  ...
  const [adminSubTab, setAdminSubTab] = useState<'management' | 'settings' | 'tegalsari' | 'marketing'>('management');
  ...
  // Tegalsari Profile Config States
  const [newSectionTitle, setNewSectionTitle] = useState('');
  const [newSectionContent, setNewSectionContent] = useState('');
  const [newSectionImg, setNewSectionImg] = useState('');
  ...
  const [settingsForm, setSettingsForm] = useState({
    ...
    tegalsari_profile: {
      ...
      phone: '',
      email: '',
      sections: [] as Array<{ id: string; title: string; content: string; image_url?: string }>,
      gallery: [] as string[]
    },
    marketing_nib_content: '',
    marketing_nib_image: '',
    marketing_pirt_content: '',
    marketing_pirt_image: '',
    marketing_halal_content: '',
    marketing_halal_image: '',
    marketing_pelatihan_content: '',
    marketing_pelatihan_image: '',
    marketing_whatsapp_content: '',
    marketing_whatsapp_image: ''
  });
  ...
  const loadAdminData = async () => {
    ...
    try {
      ...
      setSettingsForm({
        ...
        tegalsari_profile: appSettings.tegalsari_profile || {
        ...
        marketing_nib_content: appSettings.marketing_nib_content || '',
        marketing_nib_image: appSettings.marketing_nib_image || '',
        marketing_pirt_content: appSettings.marketing_pirt_content || '',
        marketing_pirt_image: appSettings.marketing_pirt_image || '',
        marketing_halal_content: appSettings.marketing_halal_content || '',
        marketing_halal_image: appSettings.marketing_halal_image || '',
        marketing_pelatihan_content: appSettings.marketing_pelatihan_content || '',
        marketing_pelatihan_image: appSettings.marketing_pelatihan_image || '',
        marketing_whatsapp_content: appSettings.marketing_whatsapp_content || '',
        marketing_whatsapp_image: appSettings.marketing_whatsapp_image || ''
      });
  ...
  const handleUpdateSettings = async (e: React.FormEvent) => {
    ...
    try {
      await db.updateAppSettings({
        ...
        tegalsari_profile: settingsForm.tegalsari_profile,
        marketing_nib_content: settingsForm.marketing_nib_content,
        marketing_nib_image: settingsForm.marketing_nib_image,
        marketing_pirt_content: settingsForm.marketing_pirt_content,
        marketing_pirt_image: settingsForm.marketing_pirt_image,
        marketing_halal_content: settingsForm.marketing_halal_content,
        marketing_halal_image: settingsForm.marketing_halal_image,
        marketing_pelatihan_content: settingsForm.marketing_pelatihan_content,
        marketing_pelatihan_image: settingsForm.marketing_pelatihan_image,
        marketing_whatsapp_content: settingsForm.marketing_whatsapp_content,
        marketing_whatsapp_image: settingsForm.marketing_whatsapp_image
      });
  ...
  const handleProfileHeroUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      const compressed = await compressImage(file, 1200, 600, 0.8);
      ...
      reader.onloadend = () => {
        if (typeof reader.result === 'string') {
          setSettingsForm(prev => ({
            ...
            tegalsari_profile: {
              ...prev.tegalsari_profile,
              hero_image: reader.result as string
            }
          }));
        }
      };
  ...
  const handleProfilePdfUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onloadend = () => {
  ...
  const handleMarketingImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, field: 'marketing_nib_image' | 'marketing_pirt_image' | 'marketing_halal_image' | 'marketing_pelatihan_image' | 'marketing_whatsapp_image') => {
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      const compressed = await compressImage(file, 800, 600, 0.8);
      setSettingsForm(prev => ({ ...prev, [field]: compressed }));
    } catch (err) {
      const reader = new FileReader();
      reader.onloadend = () => {
        if (typeof reader.result === 'string') {
  ...
  const handleAddProfileSection = () => {
    if (!newSectionTitle.trim() || !newSectionContent.trim()) {
      alert('Judul dan isi fokus wilayah harus diisi!');
      return;
    }
    const newSec = {
      id: 'sec_' + Date.now(),
      ...
      image_url: newSectionImg.trim() || undefined
    };
    setSettingsForm(prev => ({
      ...prev,
      tegalsari_profile: {
        ...prev.tegalsari_profile,
        sections: [...(prev.tegalsari_profile.sections || []), newSec]
      }
    }));
    setNewSectionTitle('');
  ...
  const handleRemoveProfileSection = (id: string) => {
    setSettingsForm(prev => ({
      ...prev,
      tegalsari_profile: {
        ...prev.tegalsari_profile,
        sections: (prev.tegalsari_profile.sections || []).filter(s => s.id !== id)
      }
    }));
  };

  const handleNewSectionImgUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      const compressed = await compressImage(file, 600, 400, 0.8);
      setNewSectionImg(compressed);
    } catch (err) {
      const reader = new FileReader();
      reader.onloadend = () => {
        if (typeof reader.result === 'string') {
          setNewSectionImg(reader.result);
        }
      };
      reader.readAsDataURL(file);
    }
  ...
  const handleAddGalleryImg = () => {
    ...
    setSettingsForm(prev => ({
      ...
      tegalsari_profile: {
        ...prev.tegalsari_profile,
        gallery: [...(prev.tegalsari_profile.gallery || []), newGalleryImg.trim()]
      }
    }));
    setNewGalleryImg('');
  };

  const handleGalleryLocalUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      ...
      setSettingsForm(prev => ({
        ...
        tegalsari_profile: {
          ...prev.tegalsari_profile,
          gallery: [...(prev.tegalsari_profile.gallery || []), compressed]
        }
      }));
    } catch (err) {
      const reader = new FileReader();
      reader.onloadend = () => {
        if (typeof reader.result === 'string') {
          setSettingsForm(prev => ({
            ...
            tegalsari_profile: {
              ...prev.tegalsari_profile,
              gallery: [...(prev.tegalsari_profile.gallery || []), reader.result]
            }
          }));
        }
      };
  ...
  const handleRemoveGalleryImg = (idx: number) => {
    setSettingsForm(prev => ({
      ...prev,
      tegalsari_profile: {
        ...prev.tegalsari_profile,
        gallery: (prev.tegalsari_profile.gallery || []).filter((_, i) => i !== idx)
      }
    }));
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-emerald-100 shadow-sm overflow-hidden p-6 space-y-8">
      ...
      <div className="flex border-b border-slate-200 gap-1.5 overflow-x-auto pb-px">
        <button
          type="button"
          onClick={() => setAdminSubTab('management')}
        ...
        >
          <Users className="w-4 h-4" /> Manajemen Warga & Vendor
        </button>
        <button
          type="button"
        ...
        >
          <Settings className="w-4 h-4" /> Pengaturan Toko & Banner
        </button>
        <button
          type="button"
          onClick={() => setAdminSubTab('tegalsari')}
          className={`px-4 py-2.5 text-xs font-bold border-b-2 transition-all cursor-pointer whitespace-nowrap flex items-center gap-1.5 ${
            adminSubTab === 'tegalsari'
              ? 'border-emerald-600 text-emerald-700'
              : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'
          }`}
        >
          <Building className="w-4 h-4" /> Profil Desa Tegalsari
        </button>
        <button
          type="button"
          onClick={() => setAdminSubTab('marketing')}
          className={`px-4 py-2.5 text-xs font-bold border-b-2 transition-all cursor-pointer whitespace-nowrap flex items-center gap-1.5 ${
        ...
        >
          <Megaphone className="w-4 h-4" /> Marketing & Perijinan Usaha
        </button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Left column: Verifying vendors & User Management */}
        <div className={`space-y-6 ${adminSubTab === 'management' ? 'lg:col-span-3' : 'hidden'}`}>
          {/* Pending vendors */}
        ...
        <div className={`space-y-6 ${adminSubTab !== 'management' ? 'lg:col-span-3' : 'hidden'}`}>
          <form onSubmit={handleUpdateSettings} className="space-y-6 text-xs">
            ...
            {/* === PENGATURAN HALAMAN PROFILE TEGALSARI === */}
            {adminSubTab === 'tegalsari' && (
              <div className="p-4 bg-amber-50/40 border border-amber-100 rounded-xl space-y-4 shadow-2xs animate-fade-in" id="admin-profile-tegalsari-settings">
                <p className="text-xs font-extrabold text-amber-900 flex items-center gap-1.5 uppercase tracking-wider font-mono">
                  🏛️ Pengaturan Konten Halaman "Profile Tegalsari"
                </p>
                <span className="text-[10px] text-amber-800/80 block leading-relaxed -mt-2">
                  Atur seluruh isi halaman company profile Desa Tegalsari, termasuk banner, berkas PDF regulasi, fokus wilayah, dan galeri foto.
                </span>

                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                  <div className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-[11px] font-bold text-slate-700 block">Judul Profil Utama</label>
                    <input
                      type="text"
                      placeholder="e.g. Profil Desa Tegalsari"
                      value={settingsForm.tegalsari_profile?.title || ''}
                      onChange={e => setSettingsForm(prev => ({
                        ...prev,
                        tegalsari_profile: {
                          ...(prev.tegalsari_profile || { title: '', subtitle: '', hero_image: '', description: '' }),
                  ...
                  <div className="space-y-1">
                    <label className="text-[11px] font-bold text-slate-700 block">Sub-Judul Profil</label>
                    <input
                      type="text"
                      placeholder="e.g. Kecamatan Tegalsari, Kabupaten Banyuwangi"
                      value={settingsForm.tegalsari_profile?.subtitle || ''}
                      onChange={e => setSettingsForm(prev => ({
                        ...prev,
                        tegalsari_profile: {
                          ...(prev.tegalsari_profile || { title: '', subtitle: '', hero_image: '', description: '' }),
                  ...
                  <div className="space-y-2">
                  <label className="text-[11px] font-bold text-slate-700 block">Foto Banner Hero Atas</label>
                  <div className="space-y-2">
                    <input
                      type="text"
                      placeholder="https://images.unsplash.com/... atau gunakan uploader di bawah"
                      value={settingsForm.tegalsari_profile?.hero_image || ''}
                      onChange={e => setSettingsForm(prev => ({
                        ...prev,
                        tegalsari_profile: {
                          ...(prev.tegalsari_profile || { title: '', subtitle: '', hero_image: '', description: '' }),
                      ...
                      className="w-full p-2 border border-slate-250 bg-white rounded-lg text-xs font-mono"
                    />
                    <label className="flex items-center justify-center gap-1.5 bg-emerald-700 hover:bg-emerald-600 text-white font-extrabold px-3 py-2 rounded-xl text-[10px] cursor-pointer shadow-sm w-full transition">
                      <UploadCloud className="w-3.5 h-3.5" />
                      Unggah Foto Banner (.jpg/.png)
                      <input
                        type="file"
                        accept="image/*"
                  ...
                  {settingsForm.tegalsari_profile?.hero_image && (
                    <img
                      src={settingsForm.tegalsari_profile.hero_image}
                      alt="Hero Preview"
                      className="h-20 w-full object-cover rounded-xl border border-slate-200 shadow-sm animate-fade-in"
                ...
                <div className="space-y-1">
                  <label className="text-[11px] font-bold text-slate-700 block">Teks Sambutan / Deskripsi Utama</label>
                  <textarea
                    rows={4}
                    placeholder="Tuliskan cerita, keunggulan, atau sambutan resmi desa di sini..."
                    value={settingsForm.tegalsari_profile?.description || ''}
                    onChange={e => setSettingsForm(prev => ({
                      ...prev,
                      tegalsari_profile: {
                        ...(prev.tegalsari_profile || { title: '', subtitle: '', hero_image: '', description: '' }),
                ...
                <div className="p-3 bg-white border border-amber-100 rounded-xl space-y-3 shadow-3xs">
                  <p className="text-[11px] font-bold text-slate-800 uppercase tracking-wide flex items-center gap-1">
                    <FileText className="w-3.5 h-3.5 text-emerald-600" /> Dokumen Regulasi / Berkas Pendukung (PDF)
                  </p>
                  
                  <div className="space-y-3">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-500 block">Nama / Label Tombol Unduh</label>
                      <input
                        type="text"
                        placeholder="e.g. Unduh Dokumen Regulasi Desa Tegalsari (PDF)"
                        value={settingsForm.tegalsari_profile?.pdf_title || ''}
                        onChange={e => setSettingsForm(prev => ({
                          ...prev,
                          tegalsari_profile: {
                            ...(prev.tegalsari_profile || { title: '', subtitle: '', hero_image: '', description: '' }),
                        ...
                        className="w-full p-2 border border-slate-250 bg-slate-50/50 rounded-lg text-xs"
                      />
                    </div>

                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-500 block">Tautan URL Berkas PDF (atau Unggah PDF)</label>
                      <div className="space-y-2">
                        <input
                          type="text"
                          placeholder="Tautan PDF eksternal..."
                          value={settingsForm.tegalsari_profile?.pdf_url || ''}
                          onChange={e => setSettingsForm(prev => ({
                            ...prev,
                            tegalsari_profile: {
                              ...(prev.tegalsari_profile || { title: '', subtitle: '', hero_image: '', description: '' }),
                          ...
                          className="w-full p-2 border border-slate-250 bg-slate-50/50 rounded-lg text-xs font-mono"
                        />
                        <label className="flex items-center justify-center gap-1.5 bg-amber-600 hover:bg-amber-500 text-white font-extrabold px-3 py-2 rounded-xl text-[10px] cursor-pointer w-full transition">
                          <UploadCloud className="w-3.5 h-3.5" />
                          Unggah File PDF Lokal
                          <input
                            type="file"
                            accept="application/pdf"
                            onChange={handleProfilePdfUpload}
                            className="hidden"
                          />
                        </label>
                      </div>
                ...
                <div className="p-3 bg-white border border-amber-100 rounded-xl space-y-3 shadow-3xs">
                  <p className="text-[11px] font-bold text-slate-800 uppercase tracking-wide flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5 text-emerald-600" /> Alamat & Kontak Resmi
                  </p>
                  
                  <div className="space-y-3">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-500 block">Alamat Sekretariat</label>
                      <input
                        type="text"
                        placeholder="e.g. Kantor Kepala Desa Tegalsari, Jl. Raya..."
                        value={settingsForm.tegalsari_profile?.address || ''}
                        onChange={e => setSettingsForm(prev => ({
                          ...prev,
                          tegalsari_profile: {
                            ...(prev.tegalsari_profile || { title: '', subtitle: '', hero_image: '', description: '' }),
                    ...
                    <div className="space-y-3">
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-slate-500 block">No. Telepon / WA</label>
                        <input
                          type="text"
                          placeholder="e.g. 6281234567890"
                          value={settingsForm.tegalsari_profile?.phone || ''}
                          onChange={e => setSettingsForm(prev => ({
                            ...prev,
                            tegalsari_profile: {
                              ...(prev.tegalsari_profile || { title: '', subtitle: '', hero_image: '', description: '' }),
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-slate-500 block">Email Resmi Desa</label>
                        <input
                          type="email"
                          placeholder="e.g. pemdes@tegalsari.id"
                          value={settingsForm.tegalsari_profile?.email || ''}
                          onChange={e => setSettingsForm(prev => ({
                            ...prev,
                            tegalsari_profile: {
                              ...(prev.tegalsari_profile || { title: '', subtitle: '', hero_image: '', description: '' }),
              ...
              <div className="space-y-4">
                ...
                <div className="p-3 bg-white border border-amber-100 rounded-xl space-y-3 shadow-3xs">
                  <p className="text-[11px] font-bold text-slate-800 uppercase tracking-wide flex items-center gap-1">
                    <Sparkles className="w-3.5 h-3.5 text-emerald-600 animate-pulse" /> Kelola Fokus & Sektor Unggulan ({(settingsForm.tegalsari_profile?.sections || []).length})
                  </p>

                  <div className="space-y-2 animate-fade-in max-h-60 overflow-y-auto">
                    {(settingsForm.tegalsari_profile?.sections || []).map((sec, i) => (
                      <div key={sec.id || i} className="flex gap-2 items-center justify-between p-2 bg-slate-50 rounded-xl border border-slate-150">
                        <div className="flex gap-2 items-center min-w-0">
                          {sec.image_url && (
                            <img
                              src={sec.image_url}
                              alt={sec.title}
                              className="w-8 h-8 object-cover rounded-lg shadow-2xs shrink-0"
                              referrerPolicy="no-referrer"
                            />
                          )}
                          <div className="min-w-0">
                            <span className="text-[10.5px] font-bold text-slate-800 block truncate">{sec.title}</span>
                            <span className="text-[9px] text-slate-500 block truncate max-w-[150px]">{sec.content}</span>
                          </div>
                        </div>
                        <button
                          type="button"
                          onClick={() => handleRemoveProfileSection(sec.id)}
                  ...
                  <div className="p-3 bg-amber-50/20 border border-dashed border-amber-200 rounded-xl space-y-3">
                    <p className="text-[9.5px] font-bold text-amber-900 uppercase">Tambah Sektor Baru</p>
                    
                    <div className="space-y-3">
                      <div className="space-y-1">
                        <label className="text-[9px] font-bold text-slate-500">Judul Sektor / Fokus</label>
                        <input
                          type="text"
                          placeholder="e.g. Visi & Misi, Pariwisata, Pertanian..."
                      ...
                      <div className="space-y-1">
                        <label className="text-[9px] font-bold text-slate-500">Foto Ilustrasi Sektor</label>
                        <div className="space-y-2">
                          <input
                            type="text"
                            placeholder="Tautan foto atau gunakan unggah di bawah..."
                            value={newSectionImg}
                          ...
                          />
                          <label className="flex items-center justify-center gap-1 bg-slate-700 hover:bg-slate-600 text-white font-extrabold px-2.5 py-2 rounded-xl text-[10px] cursor-pointer w-full transition">
                            <UploadCloud className="w-3.5 h-3.5" />
                            Unggah Foto Sektor
                            <input
                              type="file"
                              accept="image/*"
                              onChange={handleNewSectionImgUpload}
                              className="hidden"
                            />
                          </label>
                        </div>
                      ...
                      <div className="space-y-1">
                        <label className="text-[9px] font-bold text-slate-500">Isi Konten Penjelasan</label>
                        <textarea
                          rows={2}
                          placeholder="Deskripsi penjelasan mendalam sektor tersebut..."
                    ...
                    <button
                      ...
                      className="w-full py-2 bg-amber-600 hover:bg-amber-500 text-white text-[11px] font-bold rounded-xl flex items-center justify-center gap-1 cursor-pointer transition shadow-sm"
                    >
                      <Plus className="w-3.5 h-3.5" />
                      Masukkan Sektor Ke List
                    </button>
                  </div>
                </div>
                ...
                <div className="p-3 bg-white border border-amber-100 rounded-xl space-y-3 shadow-3xs">
                  <p className="text-[11px] font-bold text-slate-800 uppercase tracking-wide flex items-center gap-1">
                    <ImageIcon className="w-3.5 h-3.5 text-emerald-600" /> Galeri Foto Wilayah ({(settingsForm.tegalsari_profile?.gallery || []).length})
                  </p>

                  <div className="grid grid-cols-4 gap-2 max-h-48 overflow-y-auto p-1 bg-slate-55 rounded-lg">
                    {(settingsForm.tegalsari_profile?.gallery || []).map((img, idx) => (
                      <div key={idx} className="group relative aspect-square rounded-lg overflow-hidden border border-slate-200 shadow-3xs bg-slate-100">
                        <img
                          src={img}
                          alt="Gallery item"
                        ...
                        <button
                          ...
                          className="absolute inset-0 bg-red-600/80 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 cursor-pointer text-xs font-bold"
                        >
                  ...
                  <div className="p-3 bg-amber-50/20 border border-dashed border-amber-200 rounded-xl space-y-3">
                    <p className="text-[9.5px] font-bold text-amber-900 uppercase">Tambah Foto Galeri</p>
                    <div className="space-y-2">
                      <input
                        type="text"
                        placeholder="Tautan URL gambar galeri..."
                        value={newGalleryImg}
                      ...
                      <div className="flex flex-col gap-2 w-full">
                        <button
                          ...
                          className="w-full py-2 bg-amber-600 hover:bg-amber-500 text-white text-[11px] font-bold rounded-xl cursor-pointer transition text-center"
                        >
                          Tambah Dari URL
                        </button>
                        <label className="flex items-center justify-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-white font-extrabold px-3 py-2 rounded-xl text-[10px] cursor-pointer w-full transition">
                          <UploadCloud className="w-3.5 h-3.5" />
                          Unggah Berkas Gambar
                          <input
                            type="file"
                            accept="image/*"
                            onChange={handleGalleryLocalUpload}
                            className="hidden"
                          />
                        </label>
                      </div>
        ...
        {/* === PENGATURAN KONTEN MARKETING & PERIJINAN USAHA === */}
        {adminSubTab === 'marketing' && (
          <div className="p-4 bg-emerald-50/20 border border-emerald-100 rounded-xl space-y-6 shadow-2xs animate-fade-in lg:col-span-3" id="admin-marketing-settings">
            <div className="border-b border-emerald-100 pb-3">
              <p className="text-xs font-extrabold text-emerald-900 flex items-center gap-1.5 uppercase tracking-wider font-mono font-bold">
                📣 Kelola Konten Halaman Marketing & Perijinan Usaha
              </p>
              <span className="text-[10px] text-emerald-800/80 block leading-relaxed mt-1">
                Halaman ini dikonfigurasi secara dinamis oleh Administrator untuk warga Tegalsari. Anda dapat mengedit konten, tautan, serta mengunggah gambar pendukung untuk setiap bagian.
              </span>
            </div>

            <div className="space-y-8">
              {/* Bagian 1: NIB (Nomor Induk Berusaha) */}
              <div className="p-4 bg-white border border-slate-100 rounded-xl space-y-4 shadow-3xs">
                <div className="flex items-center gap-2 border-b border-slate-150 pb-2">
                  <div className="w-6 h-6 rounded-lg bg-emerald-100 flex items-center justify-center text-emerald-700 font-extrabold text-xs">1</div>
                  <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5 uppercase">
                    <Award className="w-4 h-4 text-emerald-600" /> Halaman Nomor Induk Berusaha (NIB)
                  </h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-2 space-y-2">
                    <label className="font-semibold text-slate-700 block">Konten Halaman (Mendukung Markdown & Tautan)</label>
                    <textarea
                      rows={6}
                      value={settingsForm.marketing_nib_content}
                      onChange={e => setSettingsForm({ ...settingsForm, marketing_nib_content: e.target.value })}
                      placeholder="Masukkan rincian informasi NIB..."
                      className="w-full p-2 border border-slate-200 bg-white rounded-lg text-xs font-mono font-medium focus:ring-1 focus:ring-emerald-500 focus:outline-none"
                    />
                  </div>
                  <div className="space-y-3">
                    <label className="font-semibold text-slate-700 block">Gambar Pendukung</label>
                    <input
                      type="text"
                      placeholder="Tautan URL Gambar..."
                      value={settingsForm.marketing_nib_image}
                      onChange={e => setSettingsForm({ ...settingsForm, marketing_nib_image: e.target.value })}
                      className="w-full p-2 border border-slate-200 bg-white rounded-lg text-xs font-mono mb-2"
                    />
                    <label className="flex items-center justify-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-white font-extrabold px-3 py-2 rounded-xl text-[10px] cursor-pointer w-full transition">
                      <UploadCloud className="w-3.5 h-3.5" />
                      Unggah Berkas Gambar
                      <input
                        type="file"
                        accept="image/*"
                        onChange={e => handleMarketingImageUpload(e, 'marketing_nib_image')}
                        className="hidden"
                      />
                    </label>
                    {settingsForm.marketing_nib_image && (
                      <div className="relative aspect-video rounded-lg overflow-hidden border border-slate-200 mt-2 bg-slate-50">
                        <img
                          src={settingsForm.marketing_nib_image}
                          alt="Preview NIB"
                          className="w-full h-full object-cover"
                          referrerPolicy="no-referrer"
                        />
              ...
              <div className="p-4 bg-white border border-slate-100 rounded-xl space-y-4 shadow-3xs">
                <div className="flex items-center gap-2 border-b border-slate-150 pb-2">
                  <div className="w-6 h-6 rounded-lg bg-emerald-100 flex items-center justify-center text-emerald-700 font-extrabold text-xs">2</div>
                  <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5 uppercase">
                    <FileText className="w-4 h-4 text-emerald-600" /> Halaman Sertifikat PIRT
                  </h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-2 space-y-2">
                    ...
                    <textarea
                      rows={6}
                      value={settingsForm.marketing_pirt_content}
                      onChange={e => setSettingsForm({ ...settingsForm, marketing_pirt_content: e.target.value })}
                      placeholder="Masukkan rincian informasi PIRT..."
                      className="w-full p-2 border border-slate-200 bg-white rounded-lg text-xs font-mono font-medium focus:ring-1 focus:ring-emerald-500 focus:outline-none"
                    />
                  </div>
                  <div className="space-y-3">
                    ...
                    <input
                      type="text"
                      placeholder="Tautan URL Gambar..."
                      value={settingsForm.marketing_pirt_image}
                      onChange={e => setSettingsForm({ ...settingsForm, marketing_pirt_image: e.target.value })}
                      className="w-full p-2 border border-slate-200 bg-white rounded-lg text-xs font-mono mb-2"
                    />
                    <label className="flex items-center justify-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-white font-extrabold px-3 py-2 rounded-xl text-[10px] cursor-pointer w-full transition">
                      <UploadCloud className="w-3.5 h-3.5" />
                      ...
                      <input
                        type="file"
                        accept="image/*"
                        onChange={e => handleMarketingImageUpload(e, 'marketing_pirt_image')}
                        className="hidden"
                      />
                    </label>
                    {settingsForm.marketing_pirt_image && (
                      <div className="relative aspect-video rounded-lg overflow-hidden border border-slate-200 mt-2 bg-slate-50">
                        <img
                          src={settingsForm.marketing_pirt_image}
                          alt="Preview PIRT"
                          className="w-full h-full object-cover"
                          referrerPolicy="no-referrer"
                        />
              ...
              {/* Bagian 3: Sertifikasi Halal */}
              <div className="p-4 bg-white border border-slate-100 rounded-xl space-y-4 shadow-3xs">
                <div className="flex items-center gap-2 border-b border-slate-150 pb-2">
                  <div className="w-6 h-6 rounded-lg bg-emerald-100 flex items-center justify-center text-emerald-700 font-extrabold text-xs">3</div>
                  <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5 uppercase">
                    <Check className="w-4 h-4 text-emerald-600" /> Halaman Sertifikasi Halal
                  </h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-2 space-y-2">
                    ...
                    <textarea
                      rows={6}
                      value={settingsForm.marketing_halal_content}
                      onChange={e => setSettingsForm({ ...settingsForm, marketing_halal_content: e.target.value })}
                      placeholder="Masukkan rincian informasi Sertifikasi Halal..."
                      className="w-full p-2 border border-slate-200 bg-white rounded-lg text-xs font-mono font-medium focus:ring-1 focus:ring-emerald-500 focus:outline-none"
                    />
                  </div>
                  <div className="space-y-3">
                    ...
                    <input
                      type="text"
                      placeholder="Tautan URL Gambar..."
                      value={settingsForm.marketing_halal_image}
                      onChange={e => setSettingsForm({ ...settingsForm, marketing_halal_image: e.target.value })}
                      className="w-full p-2 border border-slate-200 bg-white rounded-lg text-xs font-mono mb-2"
                    />
                    <label className="flex items-center justify-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-white font-extrabold px-3 py-2 rounded-xl text-[10px] cursor-pointer w-full transition">
                      <UploadCloud className="w-3.5 h-3.5" />
                      ...
                      <input
                        type="file"
                        accept="image/*"
                        onChange={e => handleMarketingImageUpload(e, 'marketing_halal_image')}
                        className="hidden"
                      />
                    </label>
                    {settingsForm.marketing_halal_image && (
                      <div className="relative aspect-video rounded-lg overflow-hidden border border-slate-200 mt-2 bg-slate-50">
                        <img
                          src={settingsForm.marketing_halal_image}
                          alt="Preview Halal"
                          className="w-full h-full object-cover"
                          referrerPolicy="no-referrer"
                        />
              ...
              {/* Bagian 4: Info Pelatihan */}
              <div className="p-4 bg-white border border-slate-100 rounded-xl space-y-4 shadow-3xs">
                <div className="flex items-center gap-2 border-b border-slate-150 pb-2">
                  <div className="w-6 h-6 rounded-lg bg-emerald-100 flex items-center justify-center text-emerald-700 font-extrabold text-xs">4</div>
                  <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5 uppercase">
                    <Zap className="w-4 h-4 text-emerald-600" /> Halaman Info Pelatihan UMKM
                  </h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-2 space-y-2">
                    ...
                    <textarea
                      rows={6}
                      value={settingsForm.marketing_pelatihan_content}
                      onChange={e => setSettingsForm({ ...settingsForm, marketing_pelatihan_content: e.target.value })}
                      placeholder="Masukkan rincian informasi pelatihan..."
                      className="w-full p-2 border border-slate-200 bg-white rounded-lg text-xs font-mono font-medium focus:ring-1 focus:ring-emerald-500 focus:outline-none"
                    />
                  </div>
                  <div className="space-y-3">
                    ...
                    <input
                      type="text"
                      placeholder="Tautan URL Gambar..."
                      value={settingsForm.marketing_pelatihan_image}
                      onChange={e => setSettingsForm({ ...settingsForm, marketing_pelatihan_image: e.target.value })}
                      className="w-full p-2 border border-slate-200 bg-white rounded-lg text-xs font-mono mb-2"
                    />
                    <label className="flex items-center justify-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-white font-extrabold px-3 py-2 rounded-xl text-[10px] cursor-pointer w-full transition">
                      <UploadCloud className="w-3.5 h-3.5" />
                      ...
                      <input
                        type="file"
                        accept="image/*"
                        onChange={e => handleMarketingImageUpload(e, 'marketing_pelatihan_image')}
                        className="hidden"
                      />
                    </label>
                    {settingsForm.marketing_pelatihan_image && (
                      <div className="relative aspect-video rounded-lg overflow-hidden border border-slate-200 mt-2 bg-slate-50">
                        <img
                          src={settingsForm.marketing_pelatihan_image}
              ...
              {/* Bagian 5: Grup WhatsApp */}
              <div className="p-4 bg-white border border-slate-100 rounded-xl space-y-4 shadow-3xs">
                <div className="flex items-center gap-2 border-b border-slate-150 pb-2">
                  <div className="w-6 h-6 rounded-lg bg-emerald-100 flex items-center justify-center text-emerald-700 font-extrabold text-xs">5</div>
                  <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5 uppercase">
                    <Users className="w-4 h-4 text-emerald-600" /> Halaman Grup WhatsApp
                  </h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-2 space-y-2">
                    ...
                    <textarea
                      rows={6}
                      value={settingsForm.marketing_whatsapp_content}
                      onChange={e => setSettingsForm({ ...settingsForm, marketing_whatsapp_content: e.target.value })}
                      placeholder="Masukkan rincian informasi dan link grup WhatsApp..."
                      className="w-full p-2 border border-slate-200 bg-white rounded-lg text-xs font-mono font-medium focus:ring-1 focus:ring-emerald-500 focus:outline-none"
                    />
                  </div>
                  <div className="space-y-3">
                    ...
                    <input
                      type="text"
                      placeholder="Tautan URL Gambar..."
                      value={settingsForm.marketing_whatsapp_image}
                      onChange={e => setSettingsForm({ ...settingsForm, marketing_whatsapp_image: e.target.value })}
                      className="w-full p-2 border border-slate-200 bg-white rounded-lg text-xs font-mono mb-2"
                    />
                    <label className="flex items-center justify-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-white font-extrabold px-3 py-2 rounded-xl text-[10px] cursor-pointer w-full transition">
                      <UploadCloud className="w-3.5 h-3.5" />
                      ...
                      <input
                        type="file"
                        accept="image/*"
                        onChange={e => handleMarketingImageUpload(e, 'marketing_whatsapp_image')}
                        className="hidden"
                      />
                    </label>
                    {settingsForm.marketing_whatsapp_image && (
                      <div className="relative aspect-video rounded-lg overflow-hidden border border-slate-200 mt-2 bg-slate-50">
                        <img
                          src={settingsForm.marketing_whatsapp_image}
      ...
      {/* Right Column of Settings Tab (Banners, Membership, Commissions) */}
      {adminSubTab === 'settings' && (
        ...
        <div className="pt-4">
          <button
            type="submit"
            disabled={loading}
            className="px-5 py-3 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white rounded-xl text-xs font-bold transition-all w-full cursor-pointer shadow-sm hover:translate-y-[-1px] flex items-center justify-center gap-2"
          >
            {loading ? 'Menyimpan...' : adminSubTab === 'tegalsari' ? 'Simpan Konten Profil Desa Tegalsari' : adminSubTab === 'marketing' ? 'Simpan Konten Marketing & Perijinan' : 'Simpan Semua Setelan Global'}
          </button>
        </div>
      </form>
    </div>
